...
export async function registrarAulaViaRequest(config: ConfigAula) {
    ...
    page.on('request', async (request) => {
        if (request.url().includes('/RegistroAula/Salvar') && request.method() === 'POST') {
            try {
                const originalPostData: any = request.postData();
            ...
            while (tentativa < maximoTentativas && !aulaRegistrada) {
                ...
                try {
                    ...
                    if (!dataAtiva.sucesso) {
                        ...
                        switch (dataAtiva.mensagem) {
                            case 'Data não encontrada!':
                                throw new Error(dataAtiva.mensagem);
                            case `Data já possui registro de aula!`:
                                aulaRegistrada = true;
                                break;
                            default:
                                await sleep(1000);
                    ...
                    await page.evaluate(async (formDataBodyString, urlReferer, urlOrigin) => {
                        ...
                        if (!response.ok) {
                            throw new Error(`Requisição POST falhou: ${response.statusText}`);
                        }
